 ...
 * @private
 */
function parse(text, language, languageOptions, filePath) {

    // TODO: Pass language in
    ...
    const file = {
        ...
        body: stripUnicodeBOM(text)
    };

    const result = language.parse(file, { options: languageOptions });

    if (result.ok) {
        return {
            success: true,
            sourceCode: language.createSourceCode(file, result, { options: languageOptions })
        };
    }
